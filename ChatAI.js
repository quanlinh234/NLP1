...
class ChatAI {
    ...
    getMessage() {
		...
		const inputElement = document.querySelector('form.message-form input');
		// Lấy giá trị đã nhập
		const inputValue = inputElement.value;
		console.log(inputValue);
        // Gửi dữ liệu chuẩn đoán       
          let data1 = {
            text: inputValue
          };
          ...
          fetch('http://localhost:3000/classify', {
            ...
            .then(data1 => {
              ...
              console.log(data1.result);
                ////////////////////////////////////////
                console.log(check);    
                if(check=="neutral"|| check =="positive")
                //switch(check)
                {
                  //  case 'neutral':
                fetch('https://api.caipacity.com/v1/chat/completions', { 
                    cache: 'no-cache',
                    method: 'POST',
                    headers: {
                ...
                if(check=="negative")
                {
                        ...
                        this.showErrorMessage("Chứa từ nhạy cảm");
                        this.container.querySelector('.message.assistant.active .blink').remove();
                        this.container.querySelector('.message-form input').disabled = false;
                        this.container.querySelector('.message.assistant.active').classList.remove('active');
